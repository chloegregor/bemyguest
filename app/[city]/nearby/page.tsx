...
export default async function City ({ params, searchParams }: { params: Promise<{ city: string }>, searchParams: Promise<{ radius: string }>}) {
  ...
  const data = await getNearByData(city_data.city as CityType, radius)
        ...
        <div className=" flex flex-col gap-5">
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-5  overflow-x-auto">
                ...
                <Link href={`/${city_params}/nearby/guests`}><p>Voir plus</p></Link>
              </div> : data.guests.length === 0 ?
              <div>
                <p className="text-[0.8em] text-gray-500">Pas de résultat.</p>
              </div> :
              <div className="flex gap-5  overflow-x-auto">
                <List data={data.guests} type={"guests"}/>
              </div>
