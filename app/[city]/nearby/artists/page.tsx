...
async function getNearByArtists(city: CityType, radius:string){
...
export default async function Artists ({ params, searchParams }: { params: Promise<{ city: string }>, searchParams: Promise<{ radius?: string }>}) {
  ...
  const {data:artists} = await getNearByArtists(city[0], radius)
  ...
  return (
    <div className="p-5 flex flex-col gap-5  ">
      ...
      {artists.length > 0 ?
      <div>
        <h1>{`Artistes résidents à proximtié de ${ville}`}</h1>
