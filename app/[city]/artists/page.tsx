...
export default async  function Artists( { params }: { params: Promise<{ city: string }> }){
  ...
  const {data:artists} =  await supabase.from('users').select('*, user_style(*, styles(*)), shop_id(*, cities(*))').eq('role', 'artist').eq('city_id', trimed_id).limit(20)
  console.log("data", artists)
  const cityName = artists?.[0]?.city_name
